...
from ex_mongo import DatabaseManager
import os
from dotenv import load_dotenv

load_dotenv()

app = FastAPI(title="MongoDB Databse API", version="1.0.0")
...
@app.post("/psots/", response_model=dict, status_code=status.HTTP_201_CREATED)
async def create_post(post: PostCreate):
    ...
    try:
        ...
        #Create if user exits
        user = db.user_collection.find_one({"_id": ObjectId(post.user_id)})
        if not user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="User not found"
...
async def get_posts_for_user(user_id: str):
    ...
    try:
        ...
        posts = db.get_posts_for_user(user_id)
        return [
            PostResponseForUser(
                id=post['_id'],
                title=post['title'],
...
async def get_all_posts():
    """Get all posts"""
    try:
        posts = list(db.post_collection.find().sort("created_at", -1))
...
async def update_post(post_id: str, title: str, content: str):
    ...
    try:
        ...
        if result.modified_count > 0:
            ...
            return {"message": "No changes made to psot"}
